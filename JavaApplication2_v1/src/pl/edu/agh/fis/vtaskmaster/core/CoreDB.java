...
public class CoreDB {
	...
	public ExecutedTask getExecutedTask(String name) throws SQLException {

        ResultSet result = statement.executeQuery("SELECT * FROM executedTasks WHERE name='"
                + name + "'");
        String taskName;
        int id;
        boolean done;
		...
		if (result.next()) {
			...
			task = new ExecutedTask(id, taskName, startTime, endTime, elapsedTime, done);
		}
        else
        {
            // there's no such task
